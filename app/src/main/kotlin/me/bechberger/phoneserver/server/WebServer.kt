...
class WebServer(private val context: Context) {
    ...
    private val huggingFaceService = me.bechberger.phoneserver.ai.HuggingFaceService(context)
    ...
    private fun Application.configureServer() {
        ...
        routing {
            ...
            // Hugging Face Model Explorer & Download
            route("/huggingface") {

                // Get recommended models
                get("/models/recommended") {
                    try {
                        val models = huggingFaceService.getRecommendedModels()
                        call.respond(mapOf(
                            "success" to true,
                        ...
                        call.respond(
                            HttpStatusCode.InternalServerError,
                            mapOf("error" to "Failed to get recommended models")
                        )
                    }
                }
                ...
                get("/models/popular") {
                    try {
                        ...
                        val models = huggingFaceService.getPopularModels(limit)
                        call.respond(mapOf(
                            "success" to true,
                            "total" to models.size,
                            "models" to models.map { model ->
                ...
                get("/models/trending") {
                    try {
                        val limit = call.request.queryParameters["limit"]?.toIntOrNull() ?: 10
                        val models = huggingFaceService.getTrendingModels(limit)
                        call.respond(mapOf(
                            "success" to true,
                            "total" to models.size,
                            "models" to models.map { model ->
                ...
                // Get mobile-optimized models
                get("/models/mobile-optimized") {
                    try {
                        val models = huggingFaceService.getMobileOptimizedModels()
                        call.respond(mapOf(
                            "success" to true,
                            "total" to models.size,
                            "note" to "These models are optimized for mobile/low-resource devices",
                            "models" to models.map { model ->
                                mapOf(
                                    "id" to model.modelId,
                                    "name" to model.name,
                ...
                get("/models/search") {
                    try {
                        ...
                        val models = huggingFaceService.searchModels(query, limit)
                        call.respond(mapOf(
                            "success" to true,
                            "query" to query,
                            "total" to models.size,
                ...
                get("/models/tag/{tag}") {
                    try {
                        val tag = call.parameters["tag"] ?: return@get call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf("error" to "Tag parameter required")
                        )

                        val models = huggingFaceService.getModelsByTag(tag)
                        call.respond(mapOf(
                            "success" to true,
                            "tag" to tag,
                            "total" to models.size,
                ...
                get("/tags") {
                    try {
                        val tags = huggingFaceService.getAvailableTags()
                        call.respond(mapOf(
                            "success" to true,
                            "total" to tags.size,
                            "tags" to tags
                ...
                get("/models/{modelId}") {
                    try {
                        val modelId = call.parameters["modelId"] ?: return@get call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf("error" to "Model ID required")
                        )

                        val model = huggingFaceService.getModelDetails(modelId)
                        if (model != null) {
                            val fileInfo = huggingFaceService.getModelFileInfo(modelId)
                            call.respond(mapOf(
                                "success" to true,
                                "model" to mapOf(
                                    "id" to model.modelId,
                ...
                // Download and install model from Hugging Face
                post("/models/download") {
                    try {
                        val request = call.receiveNullable<Map<String, String>>()
                        val modelId = request?.get("modelId") ?: return@post call.respond(
                            HttpStatusCode.BadRequest,
                            mapOf("error" to "modelId parameter required")
                        )

                        // Validate model exists
                        if (!huggingFaceService.validateModelExists(modelId)) {
                            call.respond(
                                HttpStatusCode.BadRequest,
                                mapOf("error" to "Model not found in recommendations")
                            )
                            return@post
                        }
                        ...
                        if (model == null) {
                            call.respond(
                                HttpStatusCode.NotFound,
                                mapOf("error" to "Could not retrieve model details")
                            )
                            return@post
                        }
                        ...
                        if (!permissionManager.hasEnhancedStoragePermissions(this@WebServer.context)) {
                            call.respond(
                                HttpStatusCode.Forbidden,
                                mapOf(
                                    "error" to "Storage permissions required",
                                    "code" to "STORAGE_PERMISSION_DENIED"
                                )
                            )
                            return@post
                        }

                        Timber.d("Starting download of model: $modelId")

                        // Start async download (this would be handled by the AIService in production)
                        call.respond(mapOf(
                            "success" to true,
                            "message" to "Model download started",
                            "modelId" to modelId,
                            "modelName" to model.name,
                            "status" to "downloading",
                            "jobId" to "job-${System.currentTimeMillis()}",
                            "estimatedSize" to "1-7GB depending on model",
                            "huggingFaceUrl" to model.url,
                            "note" to "Check /ai/models/loading-status for progress"
                        ))
                ...
                // Help endpoint for Hugging Face integration
                get("/help") {
                    call.respond(mapOf(
                        "title" to "Hugging Face Model Explorer API",
                        "description" to "Browse and download models from Hugging Face directly",
                        "baseUrl" to "http://device-ip:8005/huggingface",
                        "endpoints" to mapOf(
                            "GET /huggingface/models/recommended" to mapOf(
                                "description" to "Get curated recommended models for mobile",
                                "response" to mapOf(
                                    "total" to 5,
                                    "models" to "Array of model objects"
                                ),
                                "sample_request" to "GET /huggingface/models/recommended"
                            ),
                            "GET /huggingface/models/popular" to mapOf(
                                "description" to "Get most downloaded models",
                                "parameters" to "limit=10 (optional)",
                                "sample_request" to "GET /huggingface/models/popular?limit=10"
                            ),
                            "GET /huggingface/models/trending" to mapOf(
                                "description" to "Get trending models by likes",
                                "sample_request" to "GET /huggingface/models/trending"
                            ),
                            "GET /huggingface/models/mobile-optimized" to mapOf(
                                "description" to "Get models optimized for mobile devices",
                                "sample_request" to "GET /huggingface/models/mobile-optimized"
                            ),
                            "GET /huggingface/models/search?q=query" to mapOf(
                                "description" to "Search models by name, description, or tags",
                                "parameters" to mapOf(
                                    "q" to "Search query (required)",
                                    "limit" to "Max results (optional, default 20)"
                                ),
                                "sample_request" to "GET /huggingface/models/search?q=chat&limit=10"
                            ),
                            "GET /huggingface/models/tag/{tag}" to mapOf(
                                "description" to "Get models with specific tag",
                                "sample_request" to "GET /huggingface/models/tag/text-generation"
                            ),
                            "GET /huggingface/tags" to mapOf(
                                "description" to "List all available tags",
                                "sample_request" to "GET /huggingface/tags"
                            ),
                            "GET /huggingface/models/{modelId}" to mapOf(
                                "description" to "Get detailed info about a model",
                                "sample_request" to "GET /huggingface/models/google%2Fgemma-2b-it"
                            ),
                            "POST /huggingface/models/download" to mapOf(
                                "description" to "Download and install a model",
                                "request_body" to mapOf(
                                    "modelId" to "Model identifier from search results"
                                ),
                                "response" to mapOf(
                                    "status" to "downloading",
                                    "jobId" to "Job identifier for progress tracking",
                                    "estimatedSize" to "Size of model to download"
                                ),
                                "sample_request" to "POST /huggingface/models/download\n{\n  \"modelId\": \"google/gemma-2b-it\"\n}"
                            )
                        ),
                        "example_workflow" to mapOf(
                            "step1" to "Search models: GET /huggingface/models/search?q=chat",
                            "step2" to "Get details: GET /huggingface/models/{modelId}",
                            "step3" to "Download: POST /huggingface/models/download",
                            "step4" to "Check progress: GET /ai/models/loading-status"
                        ),
                        "supported_models" to mapOf(
                            "tinyllama" to "Small and fast for mobile",
                            "gemma" to "Google's lightweight model",
                            "mistral" to "Fast 7B model",
                            "phi" to "Microsoft's efficient model",
                            "llama2" to "Meta's larger model (requires auth)"
                        )
                    ))
                }
            }
